...
(function(window,document,Laya){
	...
	*@author ww
	*/
	//class code.ClassCreater
	var ClassCreater=(function(){
		function ClassCreater(){}
		__class(ClassCreater,'code.ClassCreater');
		ClassCreater.init=function(){
			ClassCreater.clzTpl=FileManager.readTxtFile(NodeJSTools.getPathByRelatviePath("data/tpl/clz.tpl"));
			ClassCreater.methodTpl=FileManager.readTxtFile(NodeJSTools.getPathByRelatviePath("data/tpl/method.tpl"));
			ClassCreater.exportPath=NodeJSTools.getPathByRelatviePath("out/clz");
		}

		ClassCreater.createClassO=function(funO){
			var funNameFull;
			...
			if (nameArr.length > 0){
				...
				filePath=FileManager.getPath(ClassCreater.exportPath,nameArr.join("/"));
				}else{
				packageStr="";
	...
	*@author ww
	*/
	//class code.CodeCreateTool
	var CodeCreateTool=(function(){
		function CodeCreateTool(){}
		__class(CodeCreateTool,'code.CodeCreateTool');
		CodeCreateTool.createExportCode=function(tplStr,data,trans){
			var rst;
			var key;
			rst=tplStr;
			var tTar;
		...
		CodeCreateTool.addPreToStr=function(str,pre){
			var lines;
			lines=str.split("\n");
			var i=0,len=0;
			len=lines.length;
	...
	*@author ww
	*/
	//class code.FunctionCreater
	var FunctionCreater=(function(){
		function FunctionCreater(){}
		__class(FunctionCreater,'code.FunctionCreater');
		FunctionCreater.init=function(){
			FunctionCreater.funTpl=FileManager.readTxtFile(NodeJSTools.getPathByRelatviePath("data/tpl/fun.tpl"));
		...
		FunctionCreater.createFunO=function(funO,createFile,tpl,importDic){
			...
			if (nameArr.length > 0){
				packageStr=nameArr.join(".");
				filePath=FileManager.getPath(FunctionCreater.exportPath,nameArr.join("/"));
				}else{
				packageStr="";
				filePath=FunctionCreater.exportPath;
			}
			dataO["package"]=packageStr;
			dataO["doc"]=FunctionCreater.createDocStr(funO,dataO);
			filePath=FileManager.getPath(filePath,dataO.name+".as");
			var codeStr;
		...
		FunctionCreater.createDocStr=function(funO,dataO){
			var docStrs;
			docStrs=[];
			docStrs.push("	/**");
		...
		FunctionCreater.createParamStr=function(paramList){
		...
		FunctionCreater.funTpl=null
		FunctionCreater.exportPath=null
		return FunctionCreater;
	})()
	...
	*/
	...
	var DesToCode=(function(){
		...
		__class(DesToCode,'DesToCode');
		var __proto=DesToCode.prototype;
	...
	*/
	//class nodetools.devices.CMDShell
	var CMDShell=(function(){
		function CMDShell(){}
		__class(CMDShell,'nodetools.devices.CMDShell');
	...
	*/
	...
	var FileTools=(function(){
		...
		FileTools.getNoVersionPath=function(tPath){
			var i=0;
			i=tPath.indexOf("?");
			if (i< 0)return tPath;
			return tPath.substr(0,i);
		}

		FileTools.getPathSep=function(tpath){
	...
	*/
	...
	var NodeJSTools=(function(){
		...
		NodeJSTools.parseParamFileToObj=function(paramFile,target){
			if (!paramFile)return;
		...
		NodeJSTools.getPathByRelatviePath=function(rPath){
			return FileManager.getPath(NodeJSTools.getMyPath(),rPath);
		}
	...
	var StringTool=(function(){
		...
		StringTool.getBetween=function(str,left,right,ifMax){
			...
			if(ifMax){
				...
				rId=str.indexOf(right,lId+left.length);
			}
			if(rId<0)return "";
			return str.substring(lId+left.length,rId);
		}

		StringTool.getPairBetweens=function(str,left,right){
			var rst=[];
			if(!str)return rst;
			if(!left)return rst;
			if(!right)return rst;
					...
					if (preI >=0){
						if (curCount > 0){
							...
							if (curCount==0){
								...
								tPair.start=preI;
								tPair.end=i;
								tPair.content=str.substring(preI,i);
								rst.push(tPair);
								preI=-1;
								curCount=0;
							}
		...
		StringTool.replaceWraped=function(str,left,right,newStr){
			var pairs;
			pairs=StringTool.getPairBetweens(str,left,right);
			if (!pairs)return str;
			var i=0,len=0;
			len=pairs.length;
			var tPair;
			for (i=0;i < len;i++){
				tPair=pairs[i];
				str=str.replace(tPair,newStr);
			}
			return str;
		}

		StringTool.getBetweenWithWrap=function(str,left,right,ifMax){
			(ifMax===void 0)&& (ifMax=false);
			var bt;
			bt=StringTool.getBetween(str,left,right,ifMax);
			return left+bt+right;
		}

		StringTool.replaceBetween=function(str,left,right,ifMax,newStr){
			(ifMax===void 0)&& (ifMax=false);
			(newStr===void 0)&& (newStr="");
			var bt;
			bt=StringTool.getBetween(str,left,right,ifMax);
			var rst;
			rst=str.replace(bt,newStr);
			return rst;
		}

		StringTool.replaceBetweenEx=function(str,left,right,ifMax,newStr){
			(ifMax===void 0)&& (ifMax=false);
			(newStr===void 0)&& (newStr="");
			var bt;
			bt=StringTool.getBetween(str,left,right,ifMax);
			var rst;
			rst=str.replace(left+bt+right,left+newStr+right);
			return rst;
		}
