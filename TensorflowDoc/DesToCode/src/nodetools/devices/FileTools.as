...
//			}
		...
		*/
		...
		public static function getNoVersionPath(tPath:String):String
		{
			var i:int;
			i = tPath.indexOf("?");
			if ( i< 0) return tPath;
		...
		public static function getPathSep(tpath:String):String
		{
		...
		public static function mkdirsSync(dirpath:String, mode:*):Boolean 
		{
			if (!fs.existsSync(dirpath)) {
				var pathtmp:String;
				var pathParts:Array = dirpath.split(getPathSep(dirpath));
				pathParts.pop();
		...
		/**创建文件*/
		public static function createFile(path:String, value:String,option:*=null):void
		{
			ensurePath(path);
			if (option)
			{
				fs.writeFileSync(path, value,option);
			}else
			{
