...
(function(window,document,Laya){
	...
	*@author ww
	*/
	//class faceai.FaceAPI
	var FaceAPI=(function(){
		function FaceAPI(){}
		__class(FaceAPI,'faceai.FaceAPI');
		FaceAPI.init=function(complete,modelPath){
			(modelPath===void 0)&& (modelPath="weights/");
			FaceAPI.faceapi=Browser.window.faceapi;
			FaceAPI.faceapi.loadFaceDetectionModel(modelPath).then(function(){
				return FaceAPI.faceapi.loadFaceLandmarkModel(modelPath);
				}).then(function(){
				if (complete)
					complete.run();
			});
		...
		FaceAPI.detectLandmarks=function(img,complete){
			FaceAPI.faceapi.detectLandmarks(img).then(function(marks){
				console.log(marks);
				if (complete){
	...
	*@author ww
	*/
	//class demo.FaceAPIDemo extends demo.PoseNetDemo
	var FaceAPIDemo=(function(_super){
		function FaceAPIDemo(){
			this._$2_sp=null;
			FaceAPIDemo.__super.call(this);
			this.useVideo=true;
		}

		__class(FaceAPIDemo,'demo.FaceAPIDemo',_super);
		var __proto=FaceAPIDemo.prototype;
		__proto.initModels=function(){
			var modelPath;
			modelPath="weights/";
			modelPath="http://10.10.20.40/weights/";
			FaceAPI.init(new Handler(this,this.modelInited),modelPath);
		}

		__proto.allInited=function(){
			this._$2_sp=new Sprite();
			this._$2_loopDetect();
		}

		__proto._$2_loopDetect=function(){
			FaceAPI.getImageFaceSprite(this.video,this._$2_sp,new Handler(this,this.onGetImageFaceSprite));
		}

		__proto.onGetImageFaceSprite=function(sp){
			Laya.stage.addChild(sp);
			sp.pos(0,0);
			Laya.timer.once(500,this,this._$2_loopDetect);
		}
	...
	*/
	...
	Laya.__init([EventDispatcher,LoaderManager,Browser,Render,Timer,LocalStorage]);
	new demo.FaceAPIDemo();
