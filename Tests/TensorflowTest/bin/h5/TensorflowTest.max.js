...
(function(window,document,Laya){
	...
	*@author ww
	*/
	//class tftool.datatool.SimpleDataSet
	var SimpleDataSet=(function(){
		...
		__class(SimpleDataSet,'tftool.datatool.SimpleDataSet');
		var __proto=SimpleDataSet.prototype;
		__proto.clear=function(){
			this.xs=null;
			this.ys=null;
		}

		__proto.addExample=function(example,label){
			var _$this=this;
			var y=tf.tidy(function(){
				return tf.oneHot(tf.tensor1d([label]).toInt(),_$this.numClasses)
			});
			if (this.xs==null){
	...
	*/
	...
	var ImageTools=(function(){
		...
		ImageTools.captureImgFromVideo=function(video,width,height){
			(width===void 0)&& (width=224);
			(height===void 0)&& (height=224);
			return ImageTools.elementToTFImage(video,width,height);
		}

		ImageTools.tfImage2Canvas=function(image,width,height,canvas){
			(width===void 0)&& (width=224);
			(height===void 0)&& (height=224);
			if (!canvas)
	...
	*@author ww
	*/
	//class demo.TransferLearningDemo extends demo.DemoBase
	var TransferLearningDemo=(function(_super){
		function TransferLearningDemo(){
			this.modelO=null;
			this.denseUnits=100;
			this.NUM_CLASSES=4;
		...
		__class(TransferLearningDemo,'demo.TransferLearningDemo',_super);
		var __proto=TransferLearningDemo.prototype;
		__proto.initModels=function(){
			var path;
			path="http://localhost/mobilenet_v1_0.25_224/model.json";
			MobileNetTool.loadMobileNet(path,new Handler(this,this.onModelLoaded),true);
		}
		...
		__proto.createTransferModel=function(){
			var layers;
			layers=[];
			layers.push(tf.layers.flatten({inputShape:[7,7,256]}));
			layers.push(tf.layers.dense({units:this.denseUnits,activation:'relu',kernelInitializer:'varianceScaling',useBias:true}));
			layers.push(tf.layers.dense({units:this.NUM_CLASSES,kernelInitializer:'varianceScaling',useBias:false,activation:'softmax'}));
			this.model=tf.sequential({layers:layers});
			var optimizer=tf.train.adam(this.learningRate);
			this.model.compile({optimizer:optimizer,loss:'categoricalCrossentropy'});
		}

		__proto.allInited=function(){
			_super.prototype.allInited.call(this);
			ImageTools.adptElementToSize(this.video);
			this.addUI();
		}
	...
	*/
	...
	//class ui.demo.TransferLearningUI extends laya.ui.View
	var TransferLearningUI=(function(_super){
		function TransferLearningUI(){
			this.trainBtn=null;
			this.playBtn=null;
			this.leftBtn=null;
			this.rightBtn=null;
	...
	*@author ww
	*/
	//class view.TransferControlView extends ui.demo.TransferLearningUI
	var TransferControlView=(function(_super){
		function TransferControlView(){
			this.video=null;
			this.dataset=null;
			this.labelCountDic={};
			this.mobileNet=null;
		...
		__class(TransferControlView,'view.TransferControlView',_super);
		var __proto=TransferControlView.prototype;
		__proto.resetSample=function(){
			this.labelCountDic={"left":0,"right":0,"up":0,"down":0 };
			this.dataset.clear();
			this.updateSampleStat();
		}

		__proto.updateSampleStat=function(){
			var strs;
			strs=[];
			var key;
			for (key in this.labelCountDic){
				strs.push(key+":"+this.labelCountDic[key]);
			}
			this.dataTxt.text=strs.join("\n");
		}

		__proto.inits=function(){
			this.dataset=new SimpleDataSet();
			var key;
			for (key in TransferControlView.dirDic){
				TransferControlView.dirStrDic[TransferControlView.dirDic[key]]=key;
			}
			this.on("click",this,this.onClick);
		}
		...
		__proto.onTrain=function(){
			var _$this=this;
			var batchSize=Math.floor(this.dataset.xs.shape[0] *this.batchsizerate);
			this.myModelO.fit(this.dataset.xs,this.dataset.ys,{batchSize:batchSize,epochs:this.epoches }).then(
			function(){
				_$this.trainComplete();
			})
		}
		...
		__proto.predict=function(){
			var _$this=this;
			var predictRst=tf.tidy(
			function(){
				var img;
				img=ImageTools.captureImgFromVideo(_$this.video);
				var activation;
				activation=_$this.mobileNet.predict(img);
				var predictions;
				predictions=_$this.myModelO.predict(activation);
				var rst=0;
		...
		__proto.addSample=function(label){
			this.labelCountDic[label]+=1;
			this.updateSampleStat();
			var labelId=0;
			labelId=TransferControlView.getDirByStr(label);
			var img;
			img=ImageTools.captureImgFromVideo(this.video);
			img=this.mobileNet.predict(img);
			this.dataset.addExample(img,labelId);
		}

		TransferControlView.getDirStr=function(dir){
			return TransferControlView.dirStrDic[dir];
		}

		TransferControlView.getDirByStr=function(dir){
			return TransferControlView.dirDic[dir];
		}
	...
	Laya.__init([LoaderManager,EventDispatcher,Timer,Browser,Render,View,GraphicAnimation,LocalStorage]);
	new demo.TransferLearningDemo();
