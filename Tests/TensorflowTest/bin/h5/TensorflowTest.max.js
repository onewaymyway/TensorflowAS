...
(function(window,document,Laya){
	...
	*@author ww
	*/
	//class tests.TestImageSprite
	var TestImageSprite=(function(){
		function TestImageSprite(){
			Laya.init(1000,900);
			this.test();
		}

		__class(TestImageSprite,'tests.TestImageSprite');
		var __proto=TestImageSprite.prototype;
	...
	*/
	...
	var ImageObject=(function(){
		...
		__class(ImageObject,'tftool.ImageObject');
		var __proto=ImageObject.prototype;
		__proto.init=function(width,height){
			this.width=width;
			this.height=height;
			this.imageData=new Browser.window.ImageData(width,height);
			this.datas=this.imageData.data;
		}

		__proto.getIndex=function(i,j){
			return (i+j *this.width)*4;
		}

		__proto.setColor=function(i,j,color){
			var index=0;
			index=this.getIndex(i,j);
			var color;
			color=Color$1.create(color);
			this.datas[index]=color._color[0] *255;
			this.datas[index+1]=color._color[1] *255;
			this.datas[index+2]=color._color[2] *255;
			this.datas[index+3]=255;
		}

		__proto.getColor=function(i,j){
			var index=0;
			index=this.getIndex(i,j);
			return ColorTool.getRGBStr([this.datas[index],this.datas[index+1],this.datas[index+2]]);
		}
	...
	*@author ww
	*/
	//class tftool.ImageSprite extends laya.display.Sprite
	var ImageSprite=(function(_super){
		function ImageSprite(){
			this.image=null;
			ImageSprite.__super.call(this);
			this.customRenderEnable=true;
		}

		__class(ImageSprite,'tftool.ImageSprite',_super);
		var __proto=ImageSprite.prototype;
		__proto.customRender=function(context,x,y){
			debugger;
			var ctx;
			ctx=context.ctx;
			if(this.image)
				ctx.putImageData(this.image.imageData,x,y);
		}
